...
class Balance extends MpesaPhpApiHttpClient
{
    /**
    ...
    public function __construct()
    {
        ...
        $this->securityCredential = $this->securityCredential(config('mpesa-php-api.intiator.credential'));

        $this->resultURL = $this->setUrl(config('mpesa-php-api.result_url.balance'));
        $this->queueTimeoutURL = $this->setUrl(config('mpesa-php-api.queue_timeout_url.balance'));
    }
    ...
    public function query($remarks, $shortCode = null)
    {
        $parameters = [
            ...
            'PartyA' => is_null($shortCode) ? config('mpesa-php-api.initiator.short_code') : $shortCode,
            'IdentifierType' => '4',
