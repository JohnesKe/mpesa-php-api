...
class Transaction extends MpesaPhpApiHttpClient
{
    /**
     * The transaction status query end point on Safaricom Mpesa API.
     *
     * @var string
     */
    ...
    public function __construct()
    {
        ...
        $this->partyA = config('mpesa-php-api.initiator.short_code');
        $this->securityCredential = $this->securityCredential(config('mpesa-php-api.initiator.credential'));

        $this->resultURL = $this->setUrl(config('mpesa-php-api.result_url.transaction_status'));
        $this->queueTimeoutURL = $this->setUrl(config('mpesa-php-api.queue_timeout_url.transaction_status'));
    }
